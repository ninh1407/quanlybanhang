...
import type { AppActionWithMeta as AppAction, AppState, WarehouseState } from '../types/app'
...
export function reducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'locations/delete':
      return {
        ...state,
        locations: removeById(state.locations, action.id),
        skuSettings: state.skuSettings.filter((s: any) => s.locationId !== action.id),
        currentLocationId: state.currentLocationId === action.id ? null : state.currentLocationId,
      }
    ...
    case 'orders/upsert':
      {
        ...
        if (!order.code || !order.code.trim()) {
          ...
          const maxExisting = state.orders.reduce((m: number, o: Order) => {
            const c = (o.code || '').trim()
    ...
    case 'orders/delete':
      return {
        ...
        stockTransactions: state.stockTransactions.filter((t: StockTransaction) => !(t.refType === 'order' && t.refId === action.id)),
        stockLedger: state.stockLedger.filter((l: StockLedgerEntry) => !(l.note.includes(action.id))), // Simplistic cleanup
        financeTransactions: state.financeTransactions.filter((t: FinanceTransaction) => !(t.refType === 'order' && t.refId === action.id)),
      }
    ...
    case 'stockVouchers/upsert':
      {
        ...
        if (!voucher.code || !voucher.code.trim()) {
          ...
          const current = sequences[seqKey] ?? 0
          const maxExisting = state.stockVouchers.reduce((m: number, v: StockVoucher) => {
            const c = (v.code || '').trim()
    ...
    case 'stockCounts/delete':
      return {
        ...
        stockTransactions: state.stockTransactions.filter(
          (t: StockTransaction) => !(t.refType === 'stock_count' && t.refId === action.id),
        ),
        financeTransactions: state.financeTransactions.filter(
          (t: FinanceTransaction) => !(t.refType === 'stock_count' && t.refId === action.id),
        ),
      }
    case 'finance/add':
      {
    ...
    case 'debts/upsert': {
      const existing = state.debts.find((d: Debt) => d.id === action.debt.id)
      if (existing) {
        return {
          ...state,
          debts: state.debts.map((d: Debt) => d.id === action.debt.id ? action.debt : d)
        }
      }
      return {
        ...state,
    ...
    case 'requests/upsert':
      {
        const exists = state.requests.some((r: InventoryRequest) => r.id === action.request.id)
    ...
    case 'notifications/markRead':
      return {
        ...state,
        notifications: state.notifications.map((n: any) => (n.id === action.id ? { ...n, read: true } : n)),
      }
    ...
    case 'skuSettings/upsert':
      {
        const exists = state.skuSettings.findIndex(
            (s: any) => s.skuId === action.setting.skuId && s.locationId === action.setting.locationId
        )
...
function orderSubTotalSimple(order: Order): number {
  ...
  return order.items.reduce((acc: number, it: OrderItem) => acc + (Number(it.qty) || 0) * (Number(it.price) || 0), 0)
}
...
export function validateAction(prev: AppState, action: AppAction): { ok: true } | { ok: false; error: string } {
  const currentUser = prev.currentUserId ? prev.users.find((u: User) => u.id === prev.currentUserId) ?? null : null
  const isAdmin = currentUser?.role === 'admin'
...
export function createAuditLog(prev: AppState, next: AppState, action: AppAction): AuditLog | null {
  ...
  if (action.type === 'requests/upsert') {
      const before = prev.requests.find((r: InventoryRequest) => r.id === action.request.id) ?? null
      const after = next.requests.find((r: InventoryRequest) => r.id === action.request.id) ?? null
  ...
  if (action.type === 'documents/upsert') {
    const before = prev.documents.find((d: AppDocument) => d.id === action.document.id) ?? null
    const after = next.documents.find((d: AppDocument) => d.id === action.document.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'documents/delete') {
    const before = prev.documents.find((d: AppDocument) => d.id === action.id) ?? null
  ...
  if (action.type === 'stockVouchers/upsert') {
    const before = prev.stockVouchers.find((x: StockVoucher) => x.id === action.voucher.id) ?? null
    const after = next.stockVouchers.find((x: StockVoucher) => x.id === action.voucher.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'stockVouchers/delete') {
    const before = prev.stockVouchers.find((x: StockVoucher) => x.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'stockVouchers/finalize') {
    const before = prev.stockVouchers.find((x: StockVoucher) => x.id === action.id) ?? null
    const after = next.stockVouchers.find((x: StockVoucher) => x.id === action.id) ?? null
  ...
  if (action.type === 'orders/upsert') {
    const before = prev.orders.find((o: Order) => o.id === action.order.id) ?? null
    const after = next.orders.find((o: Order) => o.id === action.order.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'orders/delete') {
    const before = prev.orders.find((o: Order) => o.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'stock/add') {
    const after = next.stockTransactions.find((t: StockTransaction) => t.id === action.tx.id) ?? null
    return {
      id: newId('log'),
      actorUserId,
      action: 'create',
  ...
  if (action.type === 'stockCounts/upsert') {
    const before = prev.stockCounts.find((s: StockCount) => s.id === action.stockCount.id) ?? null
    const after = next.stockCounts.find((s: StockCount) => s.id === action.stockCount.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'stockCounts/delete') {
    const before = prev.stockCounts.find((s: StockCount) => s.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'finance/add') {
    const after = next.financeTransactions.find((t: FinanceTransaction) => t.id === action.tx.id) ?? null
    return {
      id: newId('log'),
      actorUserId,
      action: 'create',
  ...
  if (action.type === 'debts/upsert') {
    const before = prev.debts.find((d: Debt) => d.id === action.debt.id) ?? null
    const after = next.debts.find((d: Debt) => d.id === action.debt.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'debts/delete') {
    const before = prev.debts.find((d: Debt) => d.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'products/upsert') {
    const before = prev.products.find((p: Product) => p.id === action.product.id) ?? null
    const after = next.products.find((p: Product) => p.id === action.product.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'skus/upsert') {
    const before = prev.skus.find((s: Sku) => s.id === action.sku.id) ?? null
    const after = next.skus.find((s: Sku) => s.id === action.sku.id) ?? null
    if (!after) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'products/delete') {
    const before = prev.products.find((p: Product) => p.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'customers/delete') {
    const before = prev.customers.find((c: Customer) => c.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'suppliers/delete') {
    const before = prev.suppliers.find((s: Supplier) => s.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'categories/delete') {
    const before = prev.categories.find((c: Category) => c.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'locations/delete') {
    const before = prev.locations.find((l: AppLocation) => l.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'skus/delete') {
    const before = prev.skus.find((s: Sku) => s.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
  ...
  if (action.type === 'users/delete') {
    const before = prev.users.find((u: User) => u.id === action.id) ?? null
    if (!before) return null
    return {
      id: newId('log'),
      actorUserId,
