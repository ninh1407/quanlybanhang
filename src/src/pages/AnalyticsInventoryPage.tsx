...
export function AnalyticsInventoryPage() {
  ...
  // Calculate composition from real data
  const compositionData = useMemo(() => {
    ...
    state.skus.forEach(s => {
        ...
        const catName = categoriesMap.get(p?.categoryId || '') || 'Khác'
        const value = qty * (s.cost || 0)
  ...
  }, [state.skus, state.products, state.categories, state.stockTransactions])

  // Calculate Top SKUs by Value from real data
  const topValuedSkus = useMemo(() => {
    const stockMap = new Map<string, number>()
    state.stockTransactions.forEach(t => {
        const delta = t.type === 'in' ? t.qty : t.type === 'out' ? -t.qty : t.qty
        stockMap.set(t.skuId, (stockMap.get(t.skuId) ?? 0) + delta)
    ...
    return state.skus
        ...
        .sort((a, b) => b.value - a.value)
        .slice(0, 10)
  }, [state.skus, state.products, state.stockTransactions])
                    ...
                    {topValuedSkus.map((item, i) => (
                        <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>SKU: {item.skuCode}</div>
                            </div>
                            <div style={{ textAlign: 'right' }}>
                                <div style={{ fontWeight: 700 }}>{formatVnd(item.value)}</div>
                                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{item.qty} chiếc</div>
                            </div>
                        </div>
                    ))}
                    {topValuedSkus.length === 0 && (
                        <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '40px 0' }}>Chưa có dữ liệu tồn kho</div>
                    )}
                </div>
            </div>
        </div>
