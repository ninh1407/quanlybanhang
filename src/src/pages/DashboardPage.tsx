...
import { format, startOfMonth, startOfDay, endOfDay, subDays, isSameMonth } from 'date-fns'
...
export function DashboardPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const serviceFee = currentOrders.reduce((sum, o) => sum + (o.otherFees || 0), 0)
    ...
    const completedOrdersCount = currentOrders.filter(o => ['delivered', 'paid'].includes(o.status)).length
    ...
    const lateOrders = 0 // Needs proper tracking in order workflow
  ...
  const employeeStats = useMemo(() => {
    const map = new Map<string, { name: string, orderCount: number, surcharge: number, shippingFee: number, total: number }>()
    const users = new Map((state.users || []).map(u => [u.id, u.fullName]))
    ...
    ;(metrics.currentOrders || []).forEach(o => {
        ...
        const curr = map.get(userId) || { name: userName, orderCount: 0, surcharge: 0, shippingFee: 0, total: 0 }
        
        map.set(userId, {
            name: userName,
            orderCount: curr.orderCount + 1,
            surcharge: curr.surcharge + (o.otherFees || 0),
            shippingFee: curr.shippingFee + (o.shippingFee || 0),
            total: curr.total + calculateOrderTotal(o)
        })
    })
  ...
  return (
    <div className="page">
      ...
      <div className="kpi-grid">
        ...
        <SmartMetricCard 
            label="Phí dịch vụ" 
            value={formatVnd(metrics.serviceFee)} 
            solid
            status="danger"
        />
        <SmartMetricCard 
      ...
      {/* 2. Employee Statistics Table - Matches User Request */}
      <div className="card" style={{ marginBottom: 20 }}>
          <div className="card-title">Thống kê theo nhân viên</div>
              ...
              <table className="table">
                  <thead>
                      <tr>
                          ...
                          <th>Nhân viên</th>
                          <th style={{ textAlign: 'right' }}>Số order</th>
                  ...
                  <tbody>
                      {employeeStats.map((item, idx) => (
                          <tr key={item.name}>
                              ...
                              <td style={{ textAlign: 'right' }}>{formatVnd(item.surcharge)}</td>
                              <td style={{ textAlign: 'right' }}>{formatVnd(item.shippingFee)}</td>
                          </tr>
                      ))}
                      {employeeStats.length === 0 && (
                          <tr>
                              <td colSpan={6} style={{ textAlign: 'center', padding: 20, color: '#6b7280' }}>Chưa có dữ liệu trong khoảng thời gian này</td>
                          </tr>
                      )}
                  </tbody>
              </table>
      ...
      {/* 3. Secondary KPI Cards (Remaining) */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20, marginBottom: 20 }}>
        <SmartMetricCard 
            label="Lợi nhuận" 
            value={apiKpi ? formatVnd(apiKpi.netProfit) : formatVnd(metrics.profit)} 
            data={apiHistory.map(h => ({ value: h.profit }))}
            status={(apiKpi?.netProfit || metrics.profit) > 0 ? 'success' : 'danger'}
        />
        <SmartMetricCard 
            label="Giá trị tồn kho" 
            value={apiKpi ? formatVnd(apiKpi.inventoryValue) : formatVnd(metrics.inventoryValue)} 
            status="warning"
        />
        <SmartMetricCard 
            label="Đơn chờ xử lý" 
        ...
        <SmartMetricCard 
            ...
            value={apiKpi ? `${apiKpi.fulfillmentRate}%` : `${metrics.fulfillmentRate.toFixed(1)}%`} 
            status="success"
        />
      </div>

      {/* 4. Revenue Trend (Full Width) */}
      <div className="card" style={{ marginBottom: 20 }}>
